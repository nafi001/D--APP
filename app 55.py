...
MAE_FILE = os.path.join(ARTIFACTS_DIR, "model_mae.pkl")
...
def feature_engineer_app(df_input, target_col_name, meteo_cols_list):
    df_fe = df_input.copy()
    # This initial block is a safeguard; primary NaT handling should occur before this function is called.
    if not isinstance(df_fe.index, pd.DatetimeIndex):
        if 'date' in df_fe.columns:
            ...
            if df_fe['date_col_temp'].isnull().any():
                 st.error("Invalid date format encountered during feature engineering (within feature_engineer_app).")
                 return None
            ...
            st.error("Date column missing for feature engineering (within feature_engineer_app).")
            return None
    df_fe.sort_index(inplace=True) # Ensure sorted index

    # If df_fe.index still contains NaT here, the error will occur.
    # The fix in run_app aims to prevent NaT in the index passed to this function.
...
def load_artifacts_cached():
    ...
    model_mae = joblib.load(MAE_FILE)
    return model, preprocessor, feature_input_cols, model_mae
...
def load_historical_data_cached(data_file_path):
    ...
    if 'date' in df_hist.columns:
        # Crucially, errors='coerce' will turn unparseable dates into NaT
...
def run_app():
    ...
    df_historical_raw = load_historical_data_cached(DATA_FILE) # df_historical_raw['date'] can contain NaT

    df_historical_app_use = df_historical_raw.copy()
    # Column name standardization already done in load_historical_data_cached if it loaded the file
    # df_historical_app_use.columns = df_historical_app_use.columns.str.replace(' ', '_').str.replace('-', '_').str.lower()
    # Date conversion to pd.Timestamp/NaT already done in load_historical_data_cached
    ...
    # Ensure we use .dt accessor for max() if 'date' column exists and is datetime-like
    if 'date' in df_historical_app_use.columns and pd.api.types.is_datetime64_any_dtype(df_historical_app_use['date']) and not df_historical_app_use['date'].dropna().empty:
        last_known_date = df_historical_app_use['date'].dropna().max()
        default_input_date = last_known_date + timedelta(days=1)
    elif 'date' in df_historical_app_use.columns and not df_historical_app_use['date'].dropna().empty : # Fallback if not datetime but not empty
        try:
            last_known_date = pd.to_datetime(df_historical_app_use['date'].dropna()).max()
            default_input_date = last_known_date + timedelta(days=1)
        except Exception: # If conversion fails, use today
             default_input_date = datetime.today()


    input_date_dt = st.sidebar.date_input("Date", value=default_input_date)
    input_date_pd = pd.to_datetime(input_date_dt) # This will be a valid Timestamp
    ...
    if st.sidebar.button("Predict 7 Days Ahead"):
        ...
        # 'date' column in new_input_df_single_row is already a valid Timestamp from input_date_pd

        base_cols_for_fe = ['date', TARGET_COL_APP] + METEO_COLS_APP
        df_hist_for_concat = df_historical_app_use.copy() # This 'date' column can have NaT
        ...
        combined_df_for_fe.set_index('date', inplace=True) # Now, index should only contain valid Timestamps
        ...
        if df_featured_full is None:
            st.error("Prediction aborted due to feature engineering error (df_featured_full is None).")
            st.stop()

        try:
            # Ensure input_date_pd (a Timestamp) is used for .loc
            input_features_for_pred_row = df_featured_full.loc[[input_date_pd]]
        except KeyError:
            st.error(f"Features for the input date {input_date_pd.strftime('%Y-%m-%d')} could not be extracted after engineering. "
                     "This might happen if the input date was removed due to being invalid or if not enough historical data exists for lags/rolling features for this specific date.")
            st.text("Tail of the feature-engineered DataFrame (before trying to extract row):")
        ...
        prediction_val = model.predict(X_predict_processed) # Renamed to avoid conflict
        predicted_cases = max(0, int(round(prediction_val[0])))

        lower_bound = max(0, int(round(predicted_cases - model_mae)))
        upper_bound = int(round(predicted_cases + model_mae))
        ...
        if 'date' in df_historical_raw.columns and pd.api.types.is_datetime64_any_dtype(df_historical_raw['date']) and not df_historical_raw['date'].dropna().empty:
             last_hist_date_in_file = df_historical_raw['date'].dropna().max()
        ...
        if is_new_date:
            ...
            if needs_retrain:
                ...
                try:
                    ...
                    df_full_updated_raw = pd.read_csv(DATA_FILE) # Load fresh data
                    ...
                    if 'date' in df_full_updated_app_use.columns:
                        df_full_updated_app_use['date'] = pd.to_datetime(df_full_updated_app_use['date'], errors='coerce')
                        # Critical: Drop NaT rows from retrain data before setting index
                        df_full_updated_app_use.dropna(subset=['date'], inplace=True) 
                        if df_full_updated_app_use.empty:
                            st.error("Retraining aborted: No valid date data after cleaning for retraining.")
                        else:
                            df_full_updated_app_use.set_index('date', inplace=True, drop=False) # Keep date column for FE
                            df_full_updated_app_use.sort_index(inplace=True)
                    else:
                        st.error("Retraining aborted: 'date' column missing in reloaded historical data.")


                    if not df_full_updated_app_use.empty and 'date' in df_full_updated_app_use.columns : # Check if still valid
                        df_fe_retrain = feature_engineer_app(df_full_updated_app_use.set_index('date'), TARGET_COL_APP, METEO_COLS_APP) # Pass with date as index
    ...
    if not current_data_for_display.empty and 'date' in current_data_for_display.columns and pd.api.types.is_datetime64_any_dtype(current_data_for_display['date']) and not current_data_for_display['date'].dropna().empty:
        min_date_display = current_data_for_display['date'].dropna().min().strftime('%Y-%m-%d')
        max_date_display = current_data_for_display['date'].dropna().max().strftime('%Y-%m-%d')
        ...
        st.write("No historical data loaded, 'date' column is problematic, or data is empty after NaT removal.")
